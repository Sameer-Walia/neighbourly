...
function MessageBoxTasker()
{
    ...
    async function fetchChats()
    {
        try
        {
            const resp = await axios.get(`${import.meta.env.VITE_API_URL}/api/chat/tasker_chat_with_all_user?taskerid=${id}`);
            if (resp.data.statuscode === 1)
            {
                setallchat(resp.data.alltaskerchat);
            }
                    ...
                    <div className="chat-list">
                        {allchat?.map((chat, index) => (
                            <div
                                ...
                                onClick={() =>
                                    navigate(
                                        `/chat?userid=${chat.userid._id}&taskerid=${id}&role=${type}`
                                    )
                                }
                            >
                                <div className="chat-left">
                                    <p className="tasker-name">{chat.userid?.name}</p>
...
export default MessageBoxTasker
